...
email varchar (40),
fkGrupo int,
constraint fkGrupo_Aluno
  foreign key (fkGrupo) references Grupo (idGrupo)
  );

-- select from join on sintase (JOIN)
...
-- inserir alunos
insert into Aluno (ra, nome_aluno, email) values
...
select * from Aluno join Grupo on Aluno.fkGrupo = Grupo.idGrupo join Avaliacao on Avaliacao.fkGrupo = idGrupo join professor on fkProfessor = idProfessor;
...
select * from Grupo join Professor on Grupo.fkAvaliar = Professor.idProfessor
    ...
    select professor.nome AS 'Nome do Professor', TRUNCATE(AVG(Avaliacao.nota),2) AS 'Media de notas', SUM(Avaliacao.nota) AS 'Soma das Notas'
    from Avaliacao join professor on idProfessor = fkProfessor group by professor.nome;